...
'Default/Standard: "RS-EPG-Update.log"
dim LogFile
LogFile="RS-EPG-Update.log"
...
For x = 0 to (objArgs.Count-1)
	...
	if lcase(left(lcase(tempargumentname), len("ini:")))=lcase("ini:") then
		...
		if ubound(arrtemp)>2 then
			inifile=""
			inifileInfoString="More than two "":"" passed in file path part of parameter /ini." & vbcrlf & "First, read the file ""readme.txt""," & vbcrlf & "then try ""cscript.exe RS-EPG-Update.vbs /ini:sample.ini""."
		end if
	end if
	TempArgumentName=""
Next
...
if inifile="" then
	if inifileInfoString<>"" then
		...
		wscript.echo "Ini file parameter has not been passed to the script or does not contain a file name, exiting." & vbcrlf & "First, read the file ""readme.txt""," & vbcrlf & "then try ""cscript.exe RS-EPG-Update.vbs /ini:sample.ini""."
	end if
	wscript.quit 1
else
	if iniFileObject.FileExists(inifile) then
		...
		wscript.echo "Ini file not found, exiting." & vbcrlf & "First, read the file ""readme.txt""," & vbcrlf & "then try ""cscript.exe RS-EPG-Update.vbs /ini:sample.ini""."
		wscript.quit 1
	end if
end if
